...
int main(int argc, char *argv[])
{
    ...
    double k3 = 0;
        ...
        // while (true)
        {
            ...
            // GaussianBlur(gray, gray, Size(1, 1),2, 2, BORDER_DEFAULT); //高斯滤波
            ...
            if (qrPointList.size() >= 3)
            {
                ...
                if (pointList.size() == 3)
                {
                    ...
                    if (jump_front > 10)
                    {
                        ...
                        cv::aruco::estimatePoseSingleMarkers(corners, marker_size, cameraMatrix, distCoeffs, rvecs, tvecs); // 求解旋转矩阵rvecs和平移矩阵tvecs
                        ...
                        myDrawDetectedMarkers(image, corners, ids, Scalar(100, 0, 255));
                        myDrawFrameAxes(image, cameraMatrix, distCoeffs, rvecs[0], tvecs[0], 50, 3);

                        imshow("threshold_output", threshold_output_copy);
                        waitKey(1);
                        sleep(0.1);
                    }
                }

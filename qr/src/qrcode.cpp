...
QList<Point> sortPoint_B(QList<Point> OutquadList, QList<Point> sortList, Point image_cen, Point A_cen)
{
    ...
    else
    {
        ...
        OutquadList_copy.removeAt(point_1);
        OutquadList_copy.removeAt(point_3);
...
QList<Point> sortPoint_C(QList<Point> OutquadList, QList<Point> sortList, Point image_cen, Point A_cen)
{
    ...
    else
    {
        ...
        sortList.push_back(OutquadList_copy[point_0]);
        sortList.push_back(OutquadList[point_1]);
        sortList.push_back(OutquadList_copy[point_2]);
        sortList.push_back(OutquadList[point_3]);
    }
    return sortList;
}
...
int main(int argc, char *argv[])
{
    ...
    // while (true)
    {
        ...
        if (qrPointList.size() >= 3)
        {
            ...
            x_A = (OutquadList[0][0].x + OutquadList[0][1].x + OutquadList[0][2].x + OutquadList[0][3].x) / float(4) + x_A;
            y_A = (OutquadList[0][0].y + OutquadList[0][1].y + OutquadList[0][2].y + OutquadList[0][3].y) / float(4) + y_A;

            x_B = (OutquadList[1][0].x + OutquadList[1][1].x + OutquadList[1][2].x + OutquadList[1][3].x) / float(4) + x_B;
            y_B = (OutquadList[1][0].y + OutquadList[1][1].y + OutquadList[1][2].y + OutquadList[1][3].y) / float(4) + y_B;

            x_C = (OutquadList[2][0].x + OutquadList[2][1].x + OutquadList[2][2].x + OutquadList[2][3].x) / float(4) + x_C;
            y_C = (OutquadList[2][0].y + OutquadList[2][1].y + OutquadList[2][2].y + OutquadList[2][3].y) / float(4) + y_C;

            x_D = float(P3_0.x + P3_1.x + P3_2.x + P3_3.x) / float(4) + x_D;
            y_D = float(P3_0.y + P3_1.y + P3_2.y + P3_3.y) / float(4) + y_D;
            sum_corner = sum_corner+1;
            }
            std::vector<cv::Vec3d> rvecs, tvecs; // 储存结果R/t
            std::vector<std::vector<cv::Point2f>> corners;
            std::vector<cv::Point2f> corner;
            float num_corner = 1;
            if (sum_corner == (num_corner))
            {
                // cout<<x_A / (num_corner)<<endl;
               
                corner.push_back(cv::Point2f(x_A / num_corner, y_A / num_corner));
                corner.push_back(cv::Point2f(x_B / num_corner, y_B / num_corner));
                corner.push_back(cv::Point2f(x_D / num_corner, y_D / num_corner));
                corner.push_back(cv::Point2f(x_C / num_corner, y_C / num_corner));
            ...
            // DrawDetectedMarkers(image, corners, 0, Scalar(100, 0, 255));
